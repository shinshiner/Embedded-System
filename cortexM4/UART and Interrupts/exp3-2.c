...
char str[50];
char cls[20] = "CLASS F1503019";
char sid[20] = "CODE 515030910468";
char cmd1[10] = "ATSTUDEN";
char cmd2[10] = "TCODE";
char cmd3[10] = "ATCLASS";
int i = 0, j, sig1 = 0, sig2 = 0, k;
...
void SysTick_Handler(void)
{
	...
	{
		systick_100ms_couter	= SYSTICK_FREQUENCY/1;
...
void output(){
		...
		UARTCharPutNonBlocking(UART0_BASE, 'M');
		UARTCharPutNonBlocking(UART0_BASE, 'E');
		UARTCharPutNonBlocking(UART0_BASE, '0' + h / 10);
		UARTCharPutNonBlocking(UART0_BASE, '0' + h % 10);
		UARTCharPutNonBlocking(UART0_BASE, ':');
		UARTCharPutNonBlocking(UART0_BASE, '0' + m / 10);
		UARTCharPutNonBlocking(UART0_BASE, '0' + m % 10);
		UARTCharPutNonBlocking(UART0_BASE, ':');
		UARTCharPutNonBlocking(UART0_BASE, '0' + s / 10);
		UARTCharPutNonBlocking(UART0_BASE, '0' + s % 10);
}
...
void UART0_Handler(void)
{
  ...
  {
		uint8_t ch = UARTCharGetNonBlocking(UART0_BASE);
		str[i] = ch;
		i++;
...
//		Delay(1000);
	...
	if(strcmp(str,"GETTIME")==0){
		output();
	}
	
	if(strlen(str)==8&&str[0]=='S'&&str[1]=='E'&&str[2]=='T'){
		tmp1 = str[3];
		tmp2 = str[4];
	...
	if(strlen(str)==8&&str[0]=='I'&&str[1]=='N'&&str[2]=='C'){
		tmp1 = str[3];
		tmp2 = str[4];
