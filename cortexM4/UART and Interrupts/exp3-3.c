...
//		Delay(1000);
	...
	if(strlen(str)!=strlen(cmd1)){
		flag1 = false;
	}
	else{
		for(k=0;k<i;++k){
			if(str[k]!=cmd1[k] && str[k]!=(cmd1[k]+'A'-'a') && str[k]!=(cmd1[k]+'a'-'A')){
				flag1 = false;
			}
		}
		flag1 = true;
	...
	if(strlen(str)!=strlen(cmd2)){
		flag2 = false;
	}
	else{
		for(k=0;k<i;++k){
			if(str[k]!=cmd2[k] && str[k]!=(cmd2[k]+'A'-'a') && str[k]!=(cmd2[k]+'a'-'A')){
				flag2 = false;
			}
		}
		flag2 = true;
	...
	if(strlen(str)!=strlen(cmd3)){
		flag3 = false;
	}
	else{
		for(k=0;k<i;++k){
			if(str[k]!=cmd3[k] && str[k]!=(cmd3[k]+'A'-'a') && str[k]!=(cmd3[k]+'a'-'A')){
				flag3 = false;
			}
		}
		flag3 = true;
	...
	if(flag2 && sig == 1){
		for(j=0;j<17;++j)
			UARTCharPutNonBlocking(UART0_BASE, sid[j]);
		sig = 0;
	}
	if(flag3){
		for(j=0;j<14;++j)
			UARTCharPutNonBlocking(UART0_BASE, cls[j]);
	}
